...
自注意力层可以说是 Transformer 和 GPT 的核心。这是输入嵌入矩阵中的列相互"交流"的阶段。
到目前为止，以及在所有其他阶段，列都可以被独立看待。

自注意力层由多个头组成，我们现在将关注其中一个。`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
第一步是从 ${c_blockRef('归一化的输入嵌入矩阵', block0.ln1.lnResid)} 的每个 ${c_dimRef('T', DimStyle.T)} 列生成三个向量。
这些向量是 Q、K 和 V 向量：
...
要生成这些向量之一，我们执行矩阵-向量乘法并添加偏置。每个
输出单元格都是输入向量的某种线性组合。例如，对于 ${c_blockRef('Q 向量', head2.qBlock)}，这是通过
${c_blockRef('Q 权重矩阵', head2.qWeightBlock)} 的一行与 ${c_blockRef('输入矩阵', block0.ln1.lnResid)} 的一列之间的点积来完成的。`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
点积运算，我们会经常看到，非常简单：我们将第一个向量的每个元素
与第二个向量的相应元素配对，将配对的元素相乘，
然后将结果相加。`;
    breakAfter();
...
这是一种通用且简单的方法，可以确保每个输出元素都能受到输入向量中所有元素的影响
（该影响由权重决定）。因此它在神经网络中频繁出现。

我们对 Q、K、V 向量中的每个输出单元格重复此操作：`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
我们如何使用 Q（查询）、K（键）和 V（值）向量？命名
给了我们一个提示："键"和"值"让人想起软件中的字典，键映射到
值。然后"查询"是我们用来查找值的东西。
...
在自注意力的情况下，我们不是返回单个条目，而是返回条目的某种加权
组合。为了找到该权重，我们在 Q 向量和每个
K 向量之间进行点积。我们对该权重进行归一化，然后最终使用它与相应的
V 向量相乘，然后将它们全部相加。
...
${embedInline((() => {
    ...
    return <div className='ml-4'>
        ...
        <div className='font-mono flex items-center my-2'>
            [w0n, w1n, w2n] =&nbsp;<span className='italic'>归一化</span>([w0, w1, w2])
        </div>
        <div className='font-mono flex items-center'>
            result =
...
对于一个更具体的例子，让我们看看第 6 列（${c_dimRef('t = 5', DimStyle.T)}），
我们将从中进行查询：`;
    breakAfter();
...
// Q (query) vector is what information is relevant to us.
    commentary(wt)`
我们查找的 {K, V} 条目是过去的 6 列，Q 值是当前时间。

我们首先计算当前列（${c_dimRef('t = 5', DimStyle.T)}）的 ${c_blockRef('Q 向量', head2.qBlock)} 与
每个先前列的 ${c_blockRef('K 向量', head2.kBlock)} 之间的点积。然后将这些点积存储在
${c_blockRef('注意力矩阵', head2.attnMtx)} 的相应行（${c_dimRef('t = 5', DimStyle.T)}）中。`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
这些点积是衡量两个向量之间相似性的一种方法。如果它们非常
相似，点积将很大。如果它们非常不同，点积将很小或
为负。

仅对过去的键使用查询的想法使这成为_因果_自注意力。也就是说，
token 不能"看到未来"。

另一个要素是，在我们进行点积之后，我们除以 sqrt(${c_dimRef('A', DimStyle.A)})，其中
${c_dimRef('A', DimStyle.A)} 是 Q/K/V 向量的长度。这种缩放是为了防止大值
在下一步的归一化（softmax）中占主导地位。

我们将主要跳过 softmax 操作（稍后描述）；只需说，每行都被归一化为总和
为 1。`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
最后，我们可以为我们的列（${c_dimRef('t = 5', DimStyle.T)}）生成输出向量。我们查看
${c_blockRef('归一化的自注意力矩阵', head2.attnMtxSm)} 的（${c_dimRef('t = 5', DimStyle.T)}）行，对于每个元素，将其他列的相应
${c_blockRef('V 向量', head2.vBlock)} 逐元素相乘。`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
然后我们可以将这些相加以生成输出向量。因此，输出向量将由
得分高的列的 V 向量主导。

现在我们知道了这个过程，让我们对所有列运行它。`;
    ...
    breakAfter();
    commentary(wt)`
这就是自注意力层的一个头的过程。因此，自注意力的主要目标是
每一列都想从其他列中找到相关信息并提取它们的值，
通过将其_查询_向量与其他列的_键_进行比较来实现。附加的限制是
它只能查看过去。
`;
